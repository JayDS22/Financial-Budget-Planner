...
function renderInvestments(){
  ...
  if(sub==='stocks'){
    ...
    html+='<div class="grid gf" style="grid-template-columns:repeat(auto-fill,minmax(270px,1fr))">'+STOCKS.map(function(s){
      ...
      h+='<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:9px"><div><div style="display:flex;align-items:center;gap:8px"><div class="mono" style="font-size:15px;font-weight:700">'+s.sym+'</div><span class="expand-icon" style="font-size:10px;color:var(--t3)">▼</span></div><div style="font-size:10px;color:var(--t3)">'+s.name+'</div></div><div class="mini-spark">'+sparkSVGForSym(s.sym,col)+'</div></div>';      h+='<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:4px"><span class="mono" style="font-size:19px;font-weight:700">$'+s.price.toFixed(2)+'</span><span class="badge" style="color:'+col+';background:'+(s.chg>=0?'var(--green-g)':'var(--red-g)')+'">'+(s.chg>=0?'+':'')+s.chg.toFixed(2)+' ('+s.pct.toFixed(2)+'%)</span></div>';
  ...
  // ===== FUNDS =====
    ...
    FUNDS.forEach(function(f){
      ...
      html+='<tr class="fund-row'+(isExp?' expanded':'')+'" onclick="toggleFund(\''+f.tick+'\')"><td style="padding:13px 16px;font-size:12px;font-weight:500">'+f.name+'</td><td class="mono" style="padding:13px 16px;color:var(--blue)">'+f.tick+'</td><td class="mono" style="padding:13px 16px">$'+f.nav.toFixed(2)+'</td><td style="padding:13px 16px;color:var(--green);font-weight:600">+'+f.ytd+'%</td><td style="padding:13px 16px;color:var(--t3)">'+f.exp+'%</td><td style="padding:13px 16px;color:var(--t3);font-size:10px"><span class="expand-icon" style="display:inline-block;transition:transform .3s;'+(isExp?'transform:rotate(180deg)':'')+'">▼</span></td></tr>';      html+='<tr><td colspan="6" style="padding:0"><div style="max-height:'+(isExp?'400px':'0')+';overflow:hidden;transition:max-height .4s ease,opacity .3s;opacity:'+(isExp?'1':'0')+'">';
      if(isExp){
  ...
  // ===== BONDS =====
    ...
    html+='<div class="grid gf">'+BONDS.map(function(b){
      ...
      h+='<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:9px"><div style="font-size:13px;font-weight:600">'+b.name+'</div><span class="expand-icon" style="font-size:10px;color:var(--t3);transition:transform .3s;'+(isExp?'transform:rotate(180deg)':'')+'">▼</span></div>';      h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:4px"><div><div style="font-size:9px;color:var(--t3);text-transform:uppercase">Yield</div><div class="mono" style="font-size:17px;font-weight:600;color:var(--green)">'+b.yld+'%</div></div><div><div style="font-size:9px;color:var(--t3);text-transform:uppercase">Price</div><div class="mono" style="font-size:17px;font-weight:600">$'+b.price+'</div></div></div>';
